...
// perticuler cell push into rowdata, row data push into new data , newdata push into data
let data = [];
let searchData;
let searchIndex;
...
// createinsidergrid function called into the create grid and index++;
...
function createInsideGrid() {
  ...
  const newData = [];
  // console.log(grid);
  //  create fisrt row of coloum and add in first line
  for (let i = 65; i <= 90; i++) {
  ...
  // create row 1-100 ittrate 100 time
  for (let i = 1; i < 100; i++) {
    createRow(i);
  }
  data.push(newData);
